...
class Member extends \Phpcmf\Model
{
    ...
    public function sendsms_text($mobile, $content, $type = 'text') {
        ...
        if ($config['third']) {
            ...
            $data['content'] = $type == 'code' ? dr_lang('您的本次验证码是: %s', $content) : $content;
            $data['content'] = '【'.$config['submailsign'].'】'.$data['content'];

            $smsapi = "https://api.mysubmail.com/message/send/";
            $data['appid'] = $config['submailapp']; //短信平台帐号
            $data['signature'] = $config['submailkey']; //短信平台密码
